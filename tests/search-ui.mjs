...
await page.addInitScript(()=>localStorage.setItem('softtask-consent-v1',JSON.stringify({version:2,analytics:false,marketing:false,expires:Date.now()+86400000})));
...
try {
  for(const route of ['/','/company/global-delivery/','/solutions/shipment-document-readiness/','/solutions/confidential-knowledge-search/','/services/software-engineering/','/industries/manufacturing/','/contact/']){
  ...
  assert.deepEqual(errors,[]);await writeFile('../../outputs/search-review/browser.json',JSON.stringify({results,errors},null,2));console.log('35 responsive checks, 21 accessibility scans, context prefill and native message validation passed.');
